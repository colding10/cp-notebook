...
template <typename T> void transpose(std::vector<std::vector<T>> &M) {
    auto tmp = M;
    M.resize(tmp[0].size());
    for (auto &row : M)
        row.resize(tmp.size());
...
public:
    ...
    template <typename T> std::vector<T> convert_back(vcd const &v) {
    ...
    template <typename T>
    std::vector<T> multiply_mod(std::vector<T> const &a, std::vector<T> const &b,
                                int const mod) {
        int result_size = a.size() + b.size() - 1;
        int size = 1 << get_lg(result_size);
...
template <> std::vector<long long> FFT::convert_back(vcd const &v) {
...
template <> std::vector<int> FFT::convert_back(vcd const &v) {
...
template <typename T> std::vector<int> to_int_vector(std::vector<T> const &v) {
    std::vector<int> w;
    w.reserve(v.size());
    for (int i = 0; i < (int)v.size(); i++)
...
template <int MOD>
std::vector<Modular<MOD>> fft_multiply(std::vector<Modular<MOD>> const &a,
std::vector<Modular<MOD>> const &b) {
    return from_int_vector<Modular<MOD>>(
               fft.multiply_mod(to_int_vector(a), to_int_vector(b), MOD));
}

template <int MOD>
std::vector<MontgomeryModular<MOD>>
                                 fft_multiply(std::vector<MontgomeryModular<MOD>> const &a,
std::vector<MontgomeryModular<MOD>> const &b) {
    return from_int_vector<MontgomeryModular<MOD>>(
               fft.multiply_mod(to_int_vector(a), to_int_vector(b), MOD));
}
