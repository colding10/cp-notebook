...
public:
    ...
    /**
     * Degree of the polynomial (acutally degree + 1, with deg = 0 for the 0
     * polynomial)
     */
    int deg() const {
        return coeffs.size();
...
private:
    std::vector<T> multiply_brute_force(std::vector<T> const &a,
                                        std::vector<T> const &b, int sz) {
    ...
    Polynomial<T> substr(int l, int r) const {
        ...
        return Polynomial<T>(
                   std::vector<T>(coeffs.begin() + l, coeffs.begin() + r));
    }
    ...
    divide_brute_force(Polynomial<T> const &g) const {
        ...
        while (divident.size() >= divisor.size()) {
            ...
            for (int i = 0; i < (int)divisor.size(); i++) {
                divident[divident.size() - i - 1] -=
                    q * divisor[divisor.size() - i - 1];
            }
            divident.pop_back();
        }
     ...
     * using binary splitting in O(n log(n)^2)
     */
    static Polynomial<T> linear_factors_product(std::vector<T> const &roots,
            int l, int r) {
        if (l + 1 == r)
            return Polynomial<T>({-roots[l], 1});
        int m = (l + r) / 2;
        return linear_factors_product(roots, l, m) *
               linear_factors_product(roots, m, r);
    }
     ...
     */
    static Polynomial<T> linear_factors_product(std::vector<T> const &roots,
            std::vector<Polynomial<T>> &tree,
            int v, int l, int r) {
        if (l + 1 == r)
            return tree[v] = Polynomial<T>({-roots[l], 1});
        int m = (l + r) / 2;
        return tree[v] = linear_factors_product(roots, tree, 2 * v, l, m) *
                         linear_factors_product(roots, tree, 2 * v + 1, m, r);
    }
     ...
     */
    std::vector<T> multi_point_evaluation(std::vector<T> const &x,
                                          std::vector<Polynomial<T>> const &tree,
                                          int v, int l, int r) const {
        if (l + 1 == r)
            return {evaluate(x[l])};

        int m = (l + r) / 2;
        std::vector<T> res1 =
            (*this % tree[2 * v]).multi_point_evaluation(x, tree, 2 * v, l, m);
        auto res2 = (*this % tree[2 * v + 1])
                    .multi_point_evaluation(x, tree, 2 * v + 1, m, r);
 ...
 */
template <typename T> T VandermondeDeterminant(std::vector<T> const &v) {
