...
template <class IterT> class StrideIter {
public:
    ...
    friend bool operator==(const StrideIter &x, const StrideIter &y) {
        return x.m == y.m;
    }
    friend bool operator!=(const StrideIter &x, const StrideIter &y) {
        return x.m != y.m;
...
template <typename IterT> class VecView {
public:
...
public:
    ...
    friend Mat strassen(Mat const &A, Mat const &B) {
        assert(A.rows == A.columns && A.columns == B.rows && B.rows == B.columns &&
               (A.rows & (A.rows - 1)) == 0);
    ...
    VecView<IterForViewConst> const column(int col_idx) const {
        ...
        auto end =
            IterForViewConst(data.cbegin() + col_idx + rows * columns, columns);
        return {beg, end};
    }
...
template <typename T> Matrix<T> power(Matrix<T> base, long long e) {
...
/**
 * Finds the shortest paths of length k given an adjacency matrix with weight
 * infos
 */
template <typename T>
