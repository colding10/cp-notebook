...
struct PrefixSum {
    ...
    PrefixSum operator+(PrefixSum const &other) {
        return {sum + other.sum, max(prefix_sum, sum + other.prefix_sum)};
    }
};
...
class MaxPrefixSegmentTree {
  public:
    ...
    void update(int idx, int add) {
        ...
        prefix_sums[idx] = {new_value, max(0, new_value)};
        while (idx > 1) {
            idx /= 2;
    ...
    long long maximum(int left, int right) { // interval [left, right)
        ...
        while (left < right) {
            if (left & 1) from_left.push_back(prefix_sums[left++]);
            if (right & 1) from_right.push_back(prefix_sums[--right]);
            left >>= 1;
            right >>= 1;
        }
